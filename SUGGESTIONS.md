...
9. Quiet-day return hook ("worst F day since May 28" historical streak). On a calm board the app has no hook to pull Dani back, so the bigger bet is a historical line that reads like "3rd Dumpster Fire this week. Worst day since May 28" straight from the BRAND.md voice rules. This needs a server-side datastore (depends on item 4): the snapshot API tracks no history, and the browser-only Hall of Shame is per-device, so it cannot honestly claim a global "worst since" streak. Hard constraint when built: the persona rejects any signup wall or notification prompt (a gate kills the free joke utility on sight), so the return hook must stay gate-free, a passive line on the page or in the share text, never a "turn on alerts" ask. Reason skipped: needs a datastore and a deploy, and a fake "worst since" line on per-device or absent history would violate data honesty. Out of scope for this pass.
...
## AAA relaunch bigger bet (logged 2026-06-19, do not block launch)
- Suspended / No-Service line hero treatment: a fully suspended line scores 50 (No Service) which lands in the PAIN TRAIN tier on the hero, so "the line is gone" reads as "annoying delays" at a glance; the NO TRAINS severity only shows one tap down in the receipts. The scoring tiers are deliberately calibrated for live-snapshot scores (Dumpster Fire reserved for 120+), so this is honest, not a bug. Bigger bet: give a Suspended/No-Service status a distinct hero stamp or color regardless of numeric tier, so a dead line reads as dead on the card. Effort S to M. No deploy dependency.
